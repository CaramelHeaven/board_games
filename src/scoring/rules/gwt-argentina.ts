...
export const gwtArgentinaFieldRules: GameFieldRules<"gwt-argentina"> = {
  ...
  buildings: {
    ...
    art: [buildingsArt],
    text: {
      ru: "Игрок получает сумму победных очков, указанных на каждом его жетоне частной постройки, размещённом на игровом поле.",
  ...
  farmers: {
    ...
    art: [farmersArt],
    text: {
      ru: "Игрок получает по 2 победных очка за каждый собранный жетон фермера перед собой (но не за те, что лежат на планшете игрока).",
  ...
  cattle: {
    ...
    text: {
      ...
      zh: "从你的牌库中（个人抽牌堆、手牌和个人弃牌堆），找出所有带有分数的疲惫卡和牛牌，取这些分数的总和。其中的负分必须扣除（因此总和也可能为负）。",
    },
  },
  objectives: {
    icon: objectivesIcon,
    text: {
      ...
      zh: "你必须找出牌库中剩余的所有目标卡。对每一张，你要决定是将它移出游戏，还是立刻放入你的个人目标区（不使用其即时效果）。之后，逐一检查个人目标区中的每张目标卡，看你是否完成了它的全部任务。对于完全完成的卡牌，累加其上标示的正分；对于未完全完成的卡牌，扣除其上标示的负分。最后按结果计分（结果也可能为负）。详见第 16 页。",
    },
  },
  stationMasters: {
    ...
    text: {
      ru: "Игрок получает сумму победных очков от индивидуальных задач жетонов начальника станции, которые лежат перед ним. Эти индивидуальные задачи (см. таблицу ниже) полностью независимы от задач на картах целей.",
